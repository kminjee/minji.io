...
5. [리액트에서 자주 사용하는 자바스크립트 문법](#리액트에서-자주-사용하는-자바스크립트-문법)
6. [선택이 아닌 필수, 타입스크립트](#선택이-아닌-필수,-타입스크립트)
...
- **배열 구조 분해 할당** <br>
  ...
  ```javascript
  // 중간 인덱스에 대한 할당을 생략하기 (단, 배열의 길이가 작을 때 쓰는 것이 좋다.)
  const array = [1,2,3,4,5]
  const [first, , , , fifth] = array

  // 기본 값 선언하기 (단, undefined이어야 기본값으로 사용된다.)
  const [a = 1, b = 1, c = 1, d = 1, e = 1] = [undefined, null, 0, '']
  // 결과) a = 1, b = null, c = 0, d = '', e = 1

  // 전개연산자를 사용해 특정값 이후의 값을 다시 배열로 선언하기
  const array = [1,2,3,4,5]
...
### 2. 전개 구문
배열이나 객체, 문자열과 같이 순회할 수 있는 값을 전개해서 사용할 수 있는 구문이다. 전에는 배열 간에 합성을 하는데 `push()`,`concat()`,`splice()`를 사용했는데 지금은 전개 구문으로 더 쉽게 배열 간 합성을 할 수 있다.

```javascript
// 배열의 전개 구문
const array1 = ['a', 'b']
const array2 = [...array1, 'c', 'd', 'e'] // ['a', 'b', 'c', 'd', 'e']

// 기존 배열에 영향을 끼치지 않고 배열 복사하기
// 값만 복사했을 뿐, 참조가 다르기 떄문에 'array1 === array2'는 false이다.
const array1 = ['a', 'b']
const array2 = [...array1]
```
...
const newObject = { ...object1, ...object2 } // { 'a': 1, 'b': 2, 'c': 3, 'd': 4 }

// 중요한 점! 객체 전개 구문은 순서가 중요하다. 
// 위와 아래에 있을 때 작동의 순서 차이로 인해 다른 객체가 생성될 수 있으므로 주의한다. 
// 값을 덮어쓸 것인지, 값을 받아들일지에 따라 순서 차이가 발생할 수 있다.
const object = { a: 1, b: 1, c: 1, : d: 1, e: 1}
const objectA = { ...object, c: 10 } // { a: 1, b: 1, c: 10, d: 1, e: 1 }
const objectB = { c: 10, ...object } // { c: 1, a: 1, b: 1, d: 1, e: 1 }
```

하지만 단순히 값을 복사하는 배열과 다르게 객체의 경우에는 객체의 속성값 및 설명자 확인과 심벌 체크 등 트랜스파일된 코드가 커진다. 번들링이 커지기 때문에 사용할 때 주의가 필요하다. 

<br>

### 3. Array 프로토타입의 메서드 (map, filter, reduce, forEach)
기존의 배열 값을 건드리지 않고 새로운 값을 만들어 내서 기존 값을 유지하고 안전하게 사용할 수 있다.

forEach의 경우 반환값이 없다. forEach에서 반환값은 undefinedfh, 콜백 함수 내부에서 아무리 반환해도 모두 의미 없는 값이 된다. <br>
또 forEach는 실행되는 순간 에러를 던지거나 프로세스를 종료하지 않는 이상 멈출 수 없다. break, return 등 어떤 것을 이용해도 배열 순회를 멈추지 않는다. return은 콜백 함수의 return으로 간주되기 때문에 멈추지 않는다. 

<br>

## 선택이 아닌 필수, 타입스크립트
타입스크립트는 동적 언어인 자바스크립트에서 런타임에만 타입을 체크할 수 있는 한계를 극복해 코드를 안전하게 작성하고 잠재적인 버그도 줄일 수 있다. 

### 1. 리액트 코드를 효과적으로 작성하기 위한 타입스크립트 활용법
- **any 대신 unknown 사용하기** <br>
  any는 불가피할 때만 사용해야 한다. any를 사용하는건 타입스크립트가 제공하는 정적 타이핑의 이점을 버리는 것이다.
  ...
  위 예제는 타입스크립트에서는 에러가 발생하지 않는다. 코드를 실행할 때 에러가 발생한다. 코드가 문제가 되는 것은 런타임이고, 타입스크립트를 사용하는 이점을 모두 없애버린다. 
  ...
  만약 타입을 단정할 수 없을 경우에는 `unknown`을 사용하는 것이 좋다. `unknown`은 모든 값을 할당할 수 있는 `top type`으로 그 어떤 값도 할당할 수 있다. 하지만 any와 다르게 바로 사용하는 것은 불가능하다. <br>
  callback()은 `unknown`, 아직 알 수 없는 값이기 때문에 사용할 수 없다는 것이여서 변수를 사용하고 싶다면 `type narrowing`, 원래 의도했던 대로 해야한다.
  ...
  top type인 unknown과 반대로 `bottom type`인 `never`이 있다. 어떠한 타입도 들어올 수 없음을 말한다. <br>
  
  ```javascript
  type what1 = string & number; // string과 number를 둘 다 만족시키는 타입은 없으므로 never 타입으로 선언됨.
  type what2 = ('hello' | 'hi') & 'react' // 두 타입에 교차점이 없으므로 never 타입으로 선언됨.
  ```
...
- **타입가드 적극 활용하기** <br>
  ...
  - `in` property in object에 사용되며, 주로 어떤 객체에 키가 존재하는지 확인하는 용도로 사용

- **제네릭** <br>
  함수나 클래스 내부에서 단일 타입이 아닌 다양한 타입에 대응할 수 있도록 도와주는 도구

- **인덱스 시그니처** <br>
  `[key: string]`을 사용한 부분이 바로 인덱스 시그니처이다. 키에 원하는 타입을 부여할 수 있어 동적인 객체를 정의할 떄 유용하지만 존재하지 않는 키로 접근하면 undefined를 반환할 수도 있다. 따라서 객체의 키는 동적으로 선언되는 경우를 최대한 지양하는 것이 좋다.
